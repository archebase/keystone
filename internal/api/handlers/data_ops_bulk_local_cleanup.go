...
func (h *DataOpsHandler) previewBulkLocalCleanup(ctx context.Context, q dataOpsEpisodeQuery) (DataOpsBulkEpisodePreviewResponse, error) {
	...
	query := `SELECT COUNT(*) AS matched_count,
		COALESCE(SUM(CASE WHEN e.cloud_synced = TRUE AND COALESCE(e.local_storage_status, 'available') <> 'deleted'
			...
			AND EXISTS (SELECT 1 FROM sync_logs sc WHERE sc.episode_id=e.id AND sc.status='completed' AND JSON_UNQUOTE(JSON_EXTRACT(sc.source_snapshot,'$.backend'))='minio' AND JSON_UNQUOTE(JSON_EXTRACT(sc.source_snapshot,'$.source_type')) IN ('original','depth_normalization')) THEN 1 ELSE 0 END),0) AS eligible_count,
