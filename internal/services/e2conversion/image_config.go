...
// ImageConfig is one immutable revision of the e2-multimodal-conversion processing settings.
...
func (m *Manager) CurrentImageConfig(ctx context.Context) (ImageConfig, error) {
	if m == nil || m.db == nil {
		return ImageConfig{}, fmt.Errorf("load E2 conversion image: database is not configured")
	}
	...
	if err := m.db.GetContext(ctx, &current, imageConfigSelect+" ORDER BY id DESC LIMIT 1"); err != nil {
		...
		return ImageConfig{}, fmt.Errorf("load current E2 conversion image: %w", err)
	}
...
func (m *Manager) UpdateImageConfig(ctx context.Context, rawImageRef string, maxConcurrent int, resourceLimitsEnabled bool, expectedRevisionID int64, actor string) (ImageConfig, error) {
	if m == nil || m.db == nil {
		return ImageConfig{}, fmt.Errorf("update E2 conversion image: database is not configured")
	}
...
func (m *Manager) ListImageConfigHistory(ctx context.Context, limit, offset int) ([]ImageConfig, error) {
	if m == nil || m.db == nil {
		return nil, fmt.Errorf("list E2 conversion image history: database is not configured")
	}
	...
	if err := m.db.SelectContext(ctx, &rows, imageConfigSelect+" ORDER BY id DESC LIMIT ? OFFSET ?", limit, offset); err != nil {
		return nil, fmt.Errorf("list E2 conversion image history: %w", err)
	}
	for i := range rows {
...
const imageConfigSelect = `
	SELECT id,
	       ...
	       created_at
	FROM e2_multimodal_conversion_image_configs`

func lockImageConfigMutex(ctx context.Context, tx *sqlx.Tx, db *sqlx.DB) error {
	query := "SELECT id FROM e2_multimodal_conversion_image_configs WHERE id = 1" + forUpdateClause(db)
	var id int64
