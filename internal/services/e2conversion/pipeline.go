...
var ErrOutputNotSettled = errors.New("E2 conversion output is not settled")

var errScratchStorageExceeded = errors.New("E2 conversion scratch storage limit exceeded")
...
// ExecutionSnapshot freezes one reusable e2-multimodal-conversion Orbit execution.
...
// VerifiedOutput is the fixed, validated output of one e2-multimodal-conversion execution.
type VerifiedOutput struct {
	...
	CalibrationObjectKey      string  `json:"calibration_object_key"`
	CalibrationSizeBytes      int64   `json:"calibration_size_bytes"`
	CalibrationChecksumSHA256 string  `json:"calibration_checksum_sha256"`
	CalibrationETag           string  `json:"calibration_etag"`
	ManifestObjectKey         string  `json:"manifest_object_key"`
	ManifestJSON              string  `json:"manifest_json"`
	ProcessingDurationSec     float64 `json:"processing_duration_sec"`
}

// PrepareExecution validates and freezes the fixed e2-multimodal-conversion Job contract.
func (m *Manager) PrepareExecution(ctx context.Context, input ExecutionInput) (ExecutionSnapshot, error) {
	if m == nil || m.db == nil {
		return ExecutionSnapshot{}, fmt.Errorf("prepare E2 conversion: database is not configured")
	}
	if !m.cfg.Enabled {
		return ExecutionSnapshot{}, ErrDisabled
	}
	if m.objects == nil {
		return ExecutionSnapshot{}, fmt.Errorf("prepare E2 conversion: TOS object reader is not configured")
	}
	...
	if err != nil {
		return ExecutionSnapshot{}, fmt.Errorf("current E2 conversion image is invalid: %w", err)
	}
	...
	if bucket == "" || objectKey == "" || outputScope == "" || submissionID == "" || input.Generation <= 0 ||
		...
		return ExecutionSnapshot{}, fmt.Errorf("invalid reusable E2 conversion execution identity")
	}
	sourceSize, sourceETag, err := m.objects.StatObject(ctx, bucket, objectKey)
	if err != nil {
		return ExecutionSnapshot{}, fmt.Errorf("stat E2 conversion source: %w", err)
	}
	...
	if err != nil {
		return ExecutionSnapshot{}, fmt.Errorf("generate E2 conversion output prefix: %w", err)
	}
	...
	if input.Calibration != nil {
		...
		if cameraSerial == "" || resultBucket == "" || resultKey == "" || resultChecksum == "" || path.Clean(resultKey) != resultKey || strings.HasPrefix(resultKey, "../") {
		...
		calibration = &CalibrationSnapshot{
			CameraSerial: cameraSerial, SessionID: sessionID, CaptureID: captureID,
			ResultURI: "tos://" + resultBucket + "/" + resultKey, ResultETag: resultETag,
			ResultSizeBytes: resultSize, ResultSHA256: resultChecksum,
		}
	}
...
func scratchStorageRequest(sourceSizeBytes int64) (string, error) {
	...
	if requiredGiB > scratchStorageLimitGiB {
		return "", fmt.Errorf(
			"%w: %w: E2 conversion source requires %dGi ephemeral storage, maximum is %dGi",
			ErrSourceUnavailable,
			errScratchStorageExceeded,
			requiredGiB,
...
type verifiedE2ConversionQA struct {
	Status      string
	Score       float64
	QualityFlag string
	Error       string
...
func (m *Manager) verifyExecutionAndQA(ctx context.Context, execution ExecutionSnapshot) (VerifiedOutput, verifiedE2ConversionQA, error) {
	output, err := m.VerifyExecution(ctx, execution)
	if err != nil {
		return VerifiedOutput{}, verifiedE2ConversionQA{}, err
	}
	var manifest processingManifest
	if err := json.Unmarshal([]byte(output.ManifestJSON), &manifest); err != nil {
		return VerifiedOutput{}, verifiedE2ConversionQA{}, fmt.Errorf("decode verified processing manifest for QA: %w", err)
	}
	qa := verifiedE2ConversionQA{Status: QAApproved, Score: 1}
	...
	if err != nil {
		return VerifiedOutput{}, verifiedE2ConversionQA{}, fmt.Errorf("encode verified E2 conversion QA result: %w", err)
	}
...
func (m *Manager) VerifyExecution(ctx context.Context, execution ExecutionSnapshot) (VerifiedOutput, error) {
	if m == nil || m.objects == nil {
		return VerifiedOutput{}, fmt.Errorf("verify E2 conversion: TOS object reader is not configured")
	}
	...
	calibrationKey := path.Join(execution.OutputPrefix, outputCalibrationName)
	calibrationETag, err := m.verifyCalibrationOutput(ctx, execution.OutputBucket, calibrationKey, manifest.Outputs.Calibration)
	if err != nil {
		return VerifiedOutput{}, err
	}
	...
	return VerifiedOutput{
		...
		CalibrationObjectKey:      calibrationKey,
		CalibrationSizeBytes:      manifest.Outputs.Calibration.SizeBytes,
		CalibrationChecksumSHA256: manifest.Outputs.Calibration.SHA256,
		CalibrationETag:           calibrationETag,
...
func (m *Manager) verifyCalibrationOutput(
	ctx context.Context,
	bucket string,
	objectKey string,
	output manifestOutput,
) (string, error) {
	etag, err := m.verifyExecutionOutputObject(ctx, bucket, objectKey, output, false)
	if err != nil {
		return "", err
	}
	body, err := m.objects.OpenObject(ctx, bucket, objectKey)
	if err != nil {
		return "", fmt.Errorf("%w: open calibration output: %v", ErrOutputNotSettled, err)
	}
	data, readErr := io.ReadAll(io.LimitReader(body, maxManifestBytes+1))
	closeErr := body.Close()
	if readErr != nil || closeErr != nil {
		return "", fmt.Errorf("%w: read calibration output: read=%v close=%v", ErrOutputNotSettled, readErr, closeErr)
	}
	if len(data) > maxManifestBytes {
		return "", fmt.Errorf("calibration output exceeds %d bytes", maxManifestBytes)
	}
	var document struct {
		Schema string `json:"schema"`
	}
	if err := json.Unmarshal(data, &document); err != nil {
		return "", fmt.Errorf("calibration output is invalid JSON: %w", err)
	}
	if document.Schema != "archebase.calibration" {
		return "", fmt.Errorf("calibration output has unexpected schema %q", document.Schema)
	}
	return etag, nil
}
