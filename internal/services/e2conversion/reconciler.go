...
// Get returns the current e2-multimodal-conversion record for an Episode.
func (m *Manager) Get(ctx context.Context, episodeID int64) (Derivative, error) {
	if m == nil || m.db == nil {
		return Derivative{}, fmt.Errorf("get E2 conversion: database is not configured")
	}
	...
	if err := m.db.GetContext(ctx, &derivative, derivativeSelect+" WHERE episode_id = ? AND kind = ?", episodeID, Kind); err != nil {
		...
		return Derivative{}, fmt.Errorf("get E2 conversion derivative: %w", err)
	}
	var payloads struct {
	...
	`, derivative.ID, Kind); err != nil {
		return Derivative{}, fmt.Errorf("get E2 conversion result payloads: %w", err)
	}
	if payloads.ProcessingResult.Valid && strings.TrimSpace(payloads.ProcessingResult.String) != "" {
		if err := json.Unmarshal([]byte(payloads.ProcessingResult.String), &derivative.ProcessingResult); err != nil {
			return Derivative{}, fmt.Errorf("decode E2 conversion processing result: %w", err)
		}
	}
	if payloads.QAResult.Valid && strings.TrimSpace(payloads.QAResult.String) != "" {
		if err := json.Unmarshal([]byte(payloads.QAResult.String), &derivative.QAResult); err != nil {
			return Derivative{}, fmt.Errorf("decode E2 conversion QA result: %w", err)
		}
	}
...
func (m *Manager) ReconcileOnce(ctx context.Context) (bool, error) {
	if m == nil || m.db == nil {
		return false, fmt.Errorf("reconcile E2 conversion: database is not configured")
	}
	if !m.cfg.Enabled {
		return false, nil
	}
	...
	if err != nil {
		...
		return false, fmt.Errorf("select E2 conversion reconcile candidate: %w", err)
	}
...
func (m *Manager) preflightQueuedScratchStorage(ctx context.Context, candidate frozenDerivativeRow) error {
	if m.objects == nil {
		return fmt.Errorf("preflight E2 conversion scratch storage: TOS object reader is not configured")
	}
	...
	`, candidate.EpisodeID); err != nil {
		...
		return fmt.Errorf("load E2 conversion source for scratch preflight: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("stat E2 conversion source for scratch preflight: %w", err)
	}
	if sourceSize <= 0 {
...
func (m *Manager) failBeforeSubmission(ctx context.Context, derivativeID int64, cause error) error {
	...
	if err != nil {
		return fmt.Errorf("persist E2 conversion pre-submission failure after %w: %w", cause, err)
	}
	rows, err := result.RowsAffected()
	if err != nil {
		return fmt.Errorf("read E2 conversion pre-submission failure result after %w: %w", cause, err)
	}
	if rows != 1 {
		return nil
	}
...
func (m *Manager) loadDispatchCapacity(ctx context.Context, includeQueued bool) (dispatchCapacity, error) {
	...
	if includeQueued {
		...
		`, Kind, ProcessingQueued, m.now().UTC()); err != nil {
			return dispatchCapacity{}, fmt.Errorf("check queued E2 conversion work: %w", err)
		}
	...
	if err != nil {
		return dispatchCapacity{}, fmt.Errorf("load E2 conversion concurrency setting: %w", err)
	}
	...
	`, Kind, ProcessingSubmitting, ProcessingPending, ProcessingRunning); err != nil {
		return dispatchCapacity{}, fmt.Errorf("count active E2 conversion jobs: %w", err)
	}
	if !includeQueued {
		return capacity, nil
	}
...
func (m *Manager) cancelRequested(ctx context.Context, derivativeID int64) (bool, error) {
	...
	`, derivativeID, Kind); err != nil {
		return false, fmt.Errorf("check E2 conversion cancellation: %w", err)
	}
...
func (m *Manager) reconcileCancellation(ctx context.Context, derivativeID int64) error {
	...
	if err := m.db.GetContext(ctx, &row, `
		...
		FROM episode_derivatives WHERE id = ? AND kind = ?
	`, derivativeID, Kind); err != nil {
		return fmt.Errorf("load E2 conversion cancellation: %w", err)
	}
...
func (m *Manager) deferCancellation(ctx context.Context, derivativeID int64, cause error) error {
	...
	`, cause.Error(), now.Add(m.pollInterval()), now, derivativeID); err != nil {
		return fmt.Errorf("persist E2 conversion cancellation retry after %v: %w", cause, err)
	}
	return cause
}
...
func (m *Manager) freezeQueued(ctx context.Context, candidate frozenDerivativeRow) error {
	if m.orbit == nil {
		return fmt.Errorf("freeze E2 conversion: Orbit is not configured")
	}
	...
	`, candidate.EpisodeID); err != nil {
		...
		return fmt.Errorf("load E2 conversion source episode: %w", err)
	}
	...
	submissionID := fmt.Sprintf("derivative-%d-e2-multimodal-conversion-g%d", candidate.ID, candidate.Generation)
	execution, err := m.PrepareExecution(ctx, ExecutionInput{
		SourceBucket:    bucket,
		SourceObjectKey: objectKey,
		SourceChecksum:  episode.Checksum.String,
		OutputScope:     path.Join(fmt.Sprintf("%d", candidate.EpisodeID), "e2-multimodal-conversion"),
	...
	if err != nil {
		return fmt.Errorf("begin E2 conversion freeze: %w", err)
	}
	...
	if err := tx.GetContext(ctx, &lockedStatus, `
		...
		return fmt.Errorf("lock E2 conversion derivative: %w", err)
	}
	...
	if err := tx.GetContext(ctx, &lockedEpisode, `
		...
		return fmt.Errorf("lock E2 conversion source episode: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("freeze E2 conversion Orbit request: %w", err)
	}
	rows, err := result.RowsAffected()
	if err != nil {
		return fmt.Errorf("read frozen E2 conversion update result: %w", err)
	}
	if rows != 1 {
		return fmt.Errorf("freeze E2 conversion Orbit request affected %d rows", rows)
	}
	if err := tx.Commit(); err != nil {
		return fmt.Errorf("commit E2 conversion freeze: %w", err)
	}
	return nil
}
...
func (m *Manager) reconcileSubmitting(ctx context.Context, derivativeID int64) error {
	...
	if err := m.db.GetContext(ctx, &frozen, `
		...
		FROM episode_derivatives WHERE id = ? AND kind = ?
	`, derivativeID, Kind); err != nil {
		return fmt.Errorf("load submitting E2 conversion: %w", err)
	}
...
func (m *Manager) failInvariant(ctx context.Context, derivativeID int64, message string) error {
	...
	if err != nil {
		return fmt.Errorf("persist E2 conversion invariant failure: %w", err)
	}
	return fmt.Errorf("E2 conversion invariant failed: %s", message)
}
...
func (m *Manager) reconcileOrbitStatus(ctx context.Context, derivativeID int64) error {
	...
	if err := m.db.GetContext(ctx, &row, `
		...
		FROM episode_derivatives WHERE id = ? AND kind = ?
	`, derivativeID, Kind); err != nil {
		return fmt.Errorf("load active E2 conversion: %w", err)
	}
	if row.CancelRequestedAt.Valid {
		return m.reconcileCancellation(ctx, derivativeID)
	}
...
type processingManifest struct {
	...
	Outputs     struct {
		...
		Calibration manifestOutput `json:"calibration"`
	} `json:"outputs"`
...
func (m *Manager) verifySucceeded(ctx context.Context, derivativeID int64) error {
	...
	if err := m.db.GetContext(ctx, &row, `
		...
		FROM episode_derivatives WHERE id = ? AND kind = ?
	`, derivativeID, Kind); err != nil {
		return fmt.Errorf("load verifying E2 conversion: %w", err)
	}
	...
	result, err := m.db.ExecContext(ctx, `
		...
		SET mcap_path = ?, metadata_path = ?, manifest_path = ?, checksum = ?,
		    ...
		    calibration_result_uri = ?, calibration_result_etag = ?, calibration_result_size_bytes = ?, calibration_result_sha256 = ?,
	...
	`, output.MCAPObjectKey, output.MetadataObjectKey, output.ManifestObjectKey, output.MCAPChecksumSHA256,
		...
		"tos://"+output.OutputBucket+"/"+output.CalibrationObjectKey, output.CalibrationETag,
		output.CalibrationSizeBytes, output.CalibrationChecksumSHA256,
	...
	if err != nil {
		return fmt.Errorf("persist verified E2 conversion output and QA: %w", err)
	}
	rows, err := result.RowsAffected()
	if err != nil {
		return fmt.Errorf("read verified E2 conversion update result: %w", err)
	}
	if rows != 1 {
...
func validateManifestSnapshot(manifest processingManifest, execution ExecutionSnapshot) error {
	if manifest.SchemaVersion != manifestSchemaV1 || manifest.Status != "succeeded" || manifest.Kind != Kind ||
		manifest.OutputFormat != e2OutputFormat || manifest.Generation != execution.Generation ||
		manifest.ProcessorImage != execution.ProcessorImage || manifest.Source.URI != execution.SourceURI ||
		manifest.Source.SizeBytes != execution.SourceSizeBytes {
		return fmt.Errorf("processing manifest does not match frozen E2 execution snapshot")
	}
	...
	if execution.Calibration != nil || manifest.Calibration != nil {
		return fmt.Errorf("E2 conversion does not accept an external calibration result")
	}
	if manifest.Outputs.MCAP.Name != outputMcapName || manifest.Outputs.Metadata.Name != outputMetadataName ||
		manifest.Outputs.Calibration.Name != "calibration.json" ||
		manifest.Outputs.MCAP.SizeBytes <= 0 || manifest.Outputs.Metadata.SizeBytes <= 0 || manifest.Outputs.Calibration.SizeBytes <= 0 ||
		normalizedSHA256(manifest.Outputs.MCAP.SHA256) == "" || normalizedSHA256(manifest.Outputs.Metadata.SHA256) == "" || normalizedSHA256(manifest.Outputs.Calibration.SHA256) == "" {
...
func validateManifestCalibration(manifest processingManifest, expected *CalibrationSnapshot) error {
	if expected != nil || manifest.Calibration != nil {
		return fmt.Errorf("E2 conversion manifest must not contain an embedded calibration")
	}
	return nil
}
...
func (m *Manager) retryVerification(ctx context.Context, row verificationRow, cause error) error {
	...
	`, cause.Error(), now.Add(m.pollInterval()), now, row.ID, ProcessingVerifying); err != nil {
		return fmt.Errorf("persist E2 conversion verification retry after %v: %w", cause, err)
	}
	return cause
}
...
func (m *Manager) reconcileQA(ctx context.Context, derivativeID int64) error {
	...
	if err := m.db.GetContext(ctx, &row, `
		...
		FROM episode_derivatives WHERE id = ? AND kind = ?
	`, derivativeID, Kind); err != nil {
		return fmt.Errorf("load E2 conversion QA input: %w", err)
	}
	...
	`, QARunning, now, now, derivativeID, QAPending, QARunning); err != nil {
		return fmt.Errorf("claim E2 conversion QA: %w", err)
	}
	...
	if !approved {
		qaStatus = QAFailed
		qaScore = 0
		qualityFlag = "E2 conversion output validation failed"
		if err != nil {
			qaError = err.Error()
		}
	}
	...
	if marshalErr != nil {
		return fmt.Errorf("encode E2 conversion QA result: %w", marshalErr)
	}
	var duration any
	...
	`, qaStatus, qaScore, qualityFlag, string(qaResult), qaError, now,
		...
		return fmt.Errorf("persist E2 conversion QA: %w", err)
	}
	return nil
}
...
func validateManifestStats(manifest processingManifest) (mcapOutputContract, error) {
	if manifest.SchemaVersion != manifestSchemaV1 || manifest.OutputFormat != e2OutputFormat {
		return mcapOutputContract{}, fmt.Errorf("invalid E2 conversion processing manifest")
	}
...
func (m *Manager) inspectOutputMCAP(
	...
	if strings.TrimSpace(objectKey) == "" {
		return mcapQAObservation{}, fmt.Errorf("E2 conversion output MCAP path is empty")
	}
	body, err := m.objects.OpenObject(ctx, m.cfg.OutputBucket, objectKey)
	if err != nil {
		return mcapQAObservation{}, fmt.Errorf("open E2 conversion output for QA: %w", err)
	}
	digest := sha256.New()
	lexer, err := mcap.NewLexer(io.TeeReader(body, digest), &mcap.LexerOptions{
		...
		AttachmentCallback: func(attachment *mcap.AttachmentReader) error {
			return fmt.Errorf("E2 conversion output must not contain attachments: %s", attachment.Name)
		},
	})
	if err != nil {
		_ = body.Close()
