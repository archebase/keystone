...
- ArcheBase 银白横向 logo 和 `生产指挥中心` 标题
- 当前日期时间，秒级更新
...
顶部品牌区应使用真实 ArcheBase logo 资产，而不是文字或临时图形替代。Logo 只作为左上角识别符号，与 `生产指挥中心` 标题构成简洁横向组合，不再额外显示 `SYNAPSE` 小标识；尺寸应随视口压缩，不得挤占中间 telemetry、右侧完整日期时间或全屏按钮。
...
`/admin/dashboard` 是深色沉浸式生产大屏，顶部品牌区应使用 ArcheBase 银白横向 logo，以保证暗屏上的远距离识别和克制质感。该替换只作用于生产大屏，不替换普通管理后台侧边栏、数据预览页或全局 `logo.png`。

品牌区由银白横向 logo 和 `生产指挥中心` 标题组成，不再显示 `SYNAPSE` 小标识。横向 logo 应按高度约束等比缩放，标题与 logo 保持同一水平视觉组，宽度响应式压缩，不得挤占中间 telemetry、完整日期时间或全屏按钮。

### 4.4 色彩与状态

建议引入 ArcheBase 品牌色作为生产大屏的主强调色，但不是把页面改成品牌营销页。大屏基底仍以工业深色、冷灰和低饱和中性色为主，品牌色只用于建立识别度和当前焦点。

Synapse 现有全局变量中已经定义 ArcheBase 品牌色：

- `--brand-blue: #07338c`
- `--brand-cyan: #079dd9`
- `--brand-yellow: #f1d86b`

大屏不应直接把这些亮色大面积铺满，而应在 `/admin/dashboard` 内派生一组暗屏 token，例如：

- `--screen-brand-primary`：用于品牌标识、当前焦点、主行动线，可从 `--brand-cyan` 或适合暗屏的品牌蓝/青混合值派生。
- `--screen-brand-strong`：用于关键数字、Video Flight Stage 边框、生命周期当前节点。
- `--screen-brand-soft`：用于轻量背景、hover、弱扫描线和刷新脉冲。
- `--screen-brand-line`：用于面板边界、图表强调线和状态轨道。

品牌强调色面积需要克制，建议只占屏幕可视面积的小比例，避免超过视觉重心。主舞台、KPI 数字、任务生命周期当前节点、趋势主数据系列可以使用品牌色；大面积背景、整卡发光、整列染色不使用品牌色。

状态色必须保持生产语义独立，不能因为品牌化而混淆：

| 状态 | 建议语义 | 使用限制 |
|---|---|---|
| 成功 / 在线 / 已完成 | 低饱和绿色 | 只用于正向状态 |
| 进行中 / 主品牌 / 当前焦点 | ArcheBase 品牌蓝 / 品牌青的暗屏派生色 | 用于主舞台边框、当前任务、重要数字 |
| 待处理 / 等待 / 警告 | 琥珀色，可参考品牌黄但需降饱和 | 不要大面积铺满，不替代警告语义 |
| 失败 / 质量异常 | 红色 | 只用于任务失败、质检失败等生产异常 |
| 设备不在线 / 工位离线 | 灰色 | 表达资源不可用，不使用红色抢占失败告警语义 |
| 中性 / 无数据 | 灰蓝 | 用于背景和非重点信息 |

图表避免默认彩虹色，使用 3 到 5 个语义色即可。近七日数据生产趋势的主系列优先使用品牌主强调色，但失败、警告、离线、空数据仍按上表语义色处理。

禁止把大屏改成紫色霓虹、全卡发光或高饱和科技风。即使后续品牌资产中出现更鲜艳的颜色，也必须先转换成适合远距离观看、暗背景对比稳定的生产大屏 token。
